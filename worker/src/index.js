...
async function createUniqueId(db) {
  for (let i = 0; i < 8; i++) {
    ...
    const existing = await db
      .prepare("SELECT id FROM members WHERE gworld_id = ?1 LIMIT 1")
      .bind(gworldId)
      .first();
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/register" && request.method === "POST") {
      ...
      try {
        const existingPhone = await env.DB
          .prepare(
            "SELECT id, gworld_id, full_name, phone, email, status FROM members WHERE phone = ?1 LIMIT 1"
          )
          .bind(phone)
          .first();
        ...
        if (existingPhone) {
          return json(
            {
              ok: false,
              error: "This phone number already has a G WORLD profile.",
              code: "PHONE_EXISTS",
            },
            409,
            request
        ...
        if (email) {
          ...
          if (existingEmail) {
            return json(
              {
                ok: false,
                error: "This email already has a G WORLD profile.",
                code: "EMAIL_EXISTS",
              },
              409,
              request
